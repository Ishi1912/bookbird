...
<body>
          ...
          <ul class="nav navbar-nav navbar-right">
              <?php
                  if(isset($_SESSION['user']))
                    ...
                    <li><a href="logout.php" class="btn btn-md"> <span class="glyphicon glyphicon-log-out">LogOut</span></a></li>
                         ';
                    }
               ?>
          </ul>
    ...
    {
        if($_POST['sort']=="price")
                {   $query = "SELECT * FROM add_products WHERE Author='$author' ORDER BY Price";
                    $result = mysqli_query ($conn,$query)or die(mysqli_error($conn));
                }
        else
        if($_POST['sort']=="priceh")
                {   $query = "SELECT * FROM add_products WHERE Author='$author' ORDER BY Price DESC";
                    $result = mysqli_query ($conn,$query)or die(mysqli_error($conn));
                }
        else
        if($_POST['sort']=="discount")
                {   $query = "SELECT * FROM add_products WHERE Author='$author' ORDER BY Discount DESC";
                    $result = mysqli_query ($conn,$query)or die(mysqli_error($conn));
                }
        else
        if($_POST['sort']=="discountl")
                {   $query = "SELECT * FROM add_products WHERE Author='$author' ORDER BY Discount";
                    $result = mysqli_query ($conn,$query)or die(mysqli_error($conn));
                }
    } 
    else   
                  $query = "SELECT * FROM add_products WHERE Author='$author'";
