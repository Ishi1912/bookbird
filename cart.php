  ...
  <section class="shopping_cart">
    ...
    <div class="cart_box_cont">
      ...
      if (mysqli_num_rows($select_cart) > 0) {
        while ($fetch_cart = mysqli_fetch_assoc($select_cart)) {
          // Determine the correct image path based on the product source
          $image_path = '';

          // Check if the product exists in the add_products table
          $check_add_products = mysqli_query($conn, "SELECT * FROM `add_products` WHERE Title = '{$fetch_cart['name']}'") or die('query failed');
          if (mysqli_num_rows($check_add_products) > 0) {
            // If product is in add_products, use its image path
            $add_product = mysqli_fetch_assoc($check_add_products);
            $image_path = "img/books/" . $add_product['PID'] . ".jpg"; // Assuming add_products images are stored in img/books/
          } else {
            // Otherwise, use the products table image path
            $image_path = "./uploaded_img/" . $fetch_cart['image'];
          }
      ?>
    ...
    <div class="cart_total">
      ...
      <div class="btns_cart">
        ...
        <a href="index.php" class="product_btn">Continue Shopping</a>
