    ...
    <section class="display_order">
        ...
        if (mysqli_num_rows($select_cart) > 0) {
            while ($fetch_cart = mysqli_fetch_assoc($select_cart)) {
                ...
                $check_add_products = mysqli_query($conn, "SELECT * FROM `add_products` WHERE Title = '$product_name'") or die('query failed');
                if (mysqli_num_rows($check_add_products) > 0) {
                    $add_product = mysqli_fetch_assoc($check_add_products);
                    $path = "img/books/" . $add_product['PID'] . ".jpg"; // Assuming add_products images are stored in img/books/
                } else {
                    $path = "./uploaded_img/" . $fetch_cart['image']; // Assuming products images are stored in uploaded_img/
                }
        ?>
